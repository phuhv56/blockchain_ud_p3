...
contract SupplyChain is FarmerRole, DistributorRole, RetailerRole, ConsumerRole {
  ...
  modifier harvested(uint _upc) {
    require(items[_upc].itemState == State.Harvested, 'Item state is not Harvested');
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Processed
  modifier processed(uint _upc) {
    require(items[_upc].itemState == State.Processed, 'Item state is not Processed');
    _;
  }
  
  // Define a modifier that checks if an item.state of a upc is Packed
  modifier packed(uint _upc) {
    require(items[_upc].itemState == State.Packed, 'Item state is not Packed');
    _;
  }
  ...
  {
    require(isFarmer(msg.sender), "Only Farmer can harvest item");
  ...
  function processItem(uint _upc) public harvested(_upc) verifyCaller(items[_upc].ownerID) {
    require(isFarmer(msg.sender), "Only Farmer can process item");
  ...
  function packItem(uint _upc) public processed(_upc) verifyCaller(items[_upc].ownerID) {
    require(isFarmer(msg.sender), "Only Farmer can pack item");
     // Update the appropriate fields
  ...
  function sellItem(uint _upc, uint _price) public packed(_upc) verifyCaller(items[_upc].ownerID) {
    require(isFarmer(msg.sender), "Only Farmer can sell item");
    // Update the appropriate fields
  ...
  function buyItem(uint _upc) public payable forSale(_upc) paidEnough(items[_upc].productPrice) checkValue(_upc) {
    require(isDistributor(msg.sender), "Only Distributor can buy item");
  ...
  function shipItem(uint _upc) public sold(_upc) verifyCaller(items[_upc].distributorID){
    require(isDistributor(msg.sender), "Only Distributor can ship item");
    // Update the appropriate fields
  ...
  function receiveItem(uint _upc) public shipped(_upc) {
    require(isRetailer(msg.sender), "Only Retailer can receive item");
  ...
  function purchaseItem(uint _upc) public received(_upc) {
    require(isConsumer(msg.sender), "Only Consumer can purchase item");
